...
```
_palette_dark.octoscript   the BASE — knobs (space_factor, radius_factor, font_base, font_step)
                       and every colour
_palette_<mood>.octoscript the DELTA — only what this mood changes; may move a knob
_derive.octoscript         every SIZE, computed from the knobs — AFTER the delta, so a mood
                       that moves a knob moves everything following from it
_kit.octoscript            the roles, which read those tokens and never a literal
```
...
assembly that omits `_derive.octoscript` leaves every size an undefined name — which is 0, so the
