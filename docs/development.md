...
that two-level budget, assignments, control flow, function returns, imported
...
completion incomplete, and disables static field hover and definition. Static
field hover and definition also fail closed when the lexical index is truncated,
because an omitted earlier reference could be a mutation.
...
and bounded nested shape spans, unique field names, direct alias binding, target,
