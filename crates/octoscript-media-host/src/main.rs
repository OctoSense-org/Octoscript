...
//! Example host: an agent-authored Octoscript workflow orchestrating two
//! policy-bound media capabilities.
//!
//! This is the "production host constructs its own reviewed catalog and policy"
//! pattern the README points to — NOT a demo baked into `octoscript-cli`. It:
//!
...
//!
//! The Octoscript source never touches ffmpeg, curl, the API key, or a filesystem
...
use octoscript_core::{check_syntax_named, ExecutionLimits};
...
fn run() -> Result<bool, String> {
    ...
    if args.len() < 2 {
        return Err(
            "usage: octoscript-media-host <draft.json> <input.json> [step:tool:max ...]".to_owned(),
        );
    }
...
/// step is valid canonical Octoscript. Effect-free — no capability host, no eval.
...
fn build_runtime(dir: PathBuf) -> Result<MobileWorkflowRuntime, String> {
    ...
    }
    {
        ...
        // GET-only, byte-bounded); the Octoscript step decides which title to query
