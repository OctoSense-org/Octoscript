...
//! Portable, capability-attenuated worker messages for Octoscript.
//!
...
///
/// This matches the maximum size of Octoscript's reviewed HTTP endpoint/origin
...
impl CapabilityGrant {
    ...
    pub fn compensation_fingerprint(&self) -> Result<String, ProtocolError> {
        ...
        hasher.update(b"octoscript-compensation-grant-v1");
...
/// opaque identifiers. It is not a Octoscript value, wire message, URL list, or
...
/// and store it through a platform-specific trusted channel, not in a Octoscript
...
/// transport, and it must never be sent through a Octoscript value, manifest,
...
fn authentication_tag(
    ...
    let encoded = serde_json::to_vec(&AuthenticationPayload {
        domain: "octoscript-worker-auth-v5",
        protocol_version: PROTOCOL_VERSION,
        sender: sender.tag(),
        sequence,
        session_id,
    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            Self::UnsupportedVersion { actual } => {
                write!(
                    formatter,
                    "unsupported Octoscript worker protocol version: {actual}"
                )
            }
...
fn worker_operation_input_fingerprint(payload: &ToolPayload) -> Result<String, ProtocolError> {
    ...
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"octoscript-worker-operation-input-v1");
    hasher.update(&input);
    Ok(hasher.finalize().to_hex().to_string())
}
