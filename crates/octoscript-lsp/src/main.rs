...
//! Stdio language server for the canonical Octoscript v0.2 source profile.
//!
...
//! Octoscript code, creates a capability host, or loads an adapter.
...
const DIAGNOSTIC_SOURCE: &str = "octoscript";
...
/// One fixed function in Octoscript's core scalar-math module.
///
...
/// One fixed scalar constant in Octoscript's core math module.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// One fixed function in Octoscript's core bounded-JSON module.
///
...
/// One fixed function in Octoscript's core bounded-text module.
///
...
/// One fixed function in Octoscript's core bounded-array module.
///
/// Like the other core tables, this is static editor metadata rather than a
/// host module catalog or a capability declaration.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// One fixed function in Octoscript's core bounded-object module.
///
/// Like the other core tables, this is static editor metadata rather than a
/// host module catalog or a capability declaration.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
const STANDARD_JSON_FUNCTIONS: &[StandardJsonFunction] = &[
    StandardJsonFunction {
        ...
        result: "JSON value",
        description:
            "Parses strict JSON from a string or UTF-8 byte array through Octoscript's configured byte and nesting limits.",
    },
    StandardJsonFunction {
        name: "stringify",
        parameters: &["value"],
        result: "string",
        description:
            "Serializes a JSON-safe value through Octoscript's configured byte, nesting, and cycle limits.",
    },
];
...
const STANDARD_MATH_AUTHORITY_NOTE: &str =
    "Effect-free Octoscript core helper; it does not access the host or grant authority.";
const STANDARD_JSON_AUTHORITY_NOTE: &str =
    "Bounded Octoscript core data helper; it does not access the host or grant authority.";
const STANDARD_TEXT_AUTHORITY_NOTE: &str =
    "Bounded Octoscript core text helper; it does not access the host or grant authority.";
const STANDARD_ARRAY_AUTHORITY_NOTE: &str =
    "Bounded Octoscript core array helper; it does not access the host or grant authority.";
const STANDARD_OBJECT_AUTHORITY_NOTE: &str =
    "Bounded Octoscript core object helper; it does not access the host or grant authority.";
...
const STANDARD_ASSERT_AUTHORITY_NOTE: &str =
    "Fixed Octoscript core helper; it does not access the host or grant authority.";
...
const NAMED_FUNCTION_SIGNATURE_DOCUMENTATION: &str = concat!(
    "Named Octoscript function declared in this document.\n\n",
    "Lexical source metadata only; it does not infer a result type, inspect a runtime, or grant authority."
);

/// One fixed path child in Octoscript's documented core import tree.
///
...
const FIXED_ROOT_MODULE_PATH_CHILDREN: &[FixedCoreModulePathChild] = &[FixedCoreModulePathChild {
    name: "std",
    detail: "Octoscript core namespace; fixed and effect-free",
    documentation:
        "Fixed Octoscript core namespace. Its presence does not resolve a host module or grant authority.",
}];

const FIXED_STANDARD_MODULE_PATH_CHILDREN: &[FixedCoreModulePathChild] = &[
    FixedCoreModulePathChild {
        name: "array",
        detail: "Octoscript core module; bounded array helpers",
        documentation:
            "Fixed Octoscript core array helpers. They perform bounded local array operations and do not access the host or grant authority.",
    },
    FixedCoreModulePathChild {
        name: "assert",
        detail: "Octoscript core function; fixed assertion helper",
        documentation: STANDARD_ASSERT_DOCUMENTATION,
    },
    FixedCoreModulePathChild {
        name: "json",
        detail: "Octoscript core module; bounded JSON helpers",
        documentation:
            "Fixed Octoscript core JSON helpers. They use the runtime's bounded JSON boundary and do not access the host or grant authority.",
    },
    FixedCoreModulePathChild {
        name: "math",
        detail: "Octoscript core module; effect-free scalar helpers",
        documentation:
            "Fixed Octoscript core scalar helpers. They do not resolve a host module or grant authority.",
    },
    FixedCoreModulePathChild {
        name: "object",
        detail: "Octoscript core module; bounded own-field helpers",
        documentation:
            "Fixed Octoscript core object helpers. They inspect bounded own record fields and do not access the host or grant authority.",
    },
    FixedCoreModulePathChild {
        name: "text",
        detail: "Octoscript core module; bounded text helpers",
        documentation:
            "Fixed Octoscript core text helpers. They use bounded string construction and do not access the host or grant authority.",
    },
];
...
#[derive(Default)]
struct OctoscriptLanguageServer {
...
impl OctoscriptLanguageServer {
    ...
    fn document_symbols(&self, uri: &Uri) -> Result<Vec<DocumentSymbol>, String> {
        ...
        let declarations =
            top_level_declarations_named(uri.as_str(), source, ExecutionLimits::default())
                .map_err(|error| format!("cannot outline canonical Octoscript source: {error}"))?;
    ...
    fn references(
        ...
        if report.truncated {
            return Err(format!(
                "the lexical index exceeds Octoscript's {MAX_LEXICAL_SYMBOL_OCCURRENCES}-occurrence limit"
            ));
        }
        let Some(byte_offset) = byte_at_position(source, position) else {
            return Ok(Vec::new());
    ...
    fn rename(
        ...
        if symbol.kind == LexicalSymbolKind::Import {
            return Err(
                "Octoscript cannot rename an import binding without changing its module path"
                    .to_owned(),
            );
        }
        ...
        if new_name.len() > DEFAULT_MAX_SOURCE_BYTES {
            return Err(format!(
                "the rename identifier exceeds Octoscript's {DEFAULT_MAX_SOURCE_BYTES}-byte source limit"
            ));
        }
        ...
        let renamed_report =
            lexical_symbol_report_named(uri.as_str(), &renamed_source, ExecutionLimits::default())
                .map_err(|error| format!("cannot index renamed Octoscript source: {error}"))?;
    ...
    fn lexical_symbols(&self, uri: &Uri) -> Result<(&str, i32, &LexicalSymbolReport), String> {
        ...
        let report = state.lexical_report.get_or_init(|| {
            lexical_symbol_report_named(uri.as_str(), source, ExecutionLimits::default())
                .map_err(|error| format!("cannot index canonical Octoscript source: {error}"))
        });
        match report {
    ...
    fn lexical_completions(&self, uri: &Uri) -> Result<(&str, &LexicalCompletionReport), String> {
        ...
        let report = state.completion_report.get_or_init(|| {
            lexical_completion_report_named(uri.as_str(), source, ExecutionLimits::default())
                .map_err(|error| format!("cannot complete canonical Octoscript source: {error}"))
        });
        match report {
    ...
    fn module_imports(&self, uri: &Uri) -> Result<&ModuleImportReport, String> {
        ...
        let report = state.module_import_report.get_or_init(|| {
            module_import_report_named(uri.as_str(), source, ExecutionLimits::default())
                .map_err(|error| format!("cannot inspect canonical Octoscript imports: {error}"))
        });
        match report {
            Ok(report) => Ok(report),
            Err(message) => Err(message.clone()),
    ...
    fn replace_document(
        ...
        if !self.documents.contains_key(&uri) && self.documents.len() >= MAX_OPEN_DOCUMENTS {
            return resource_diagnostics(
                ...
                version,
                format!(
                    "Octoscript LSP tracks at most {MAX_OPEN_DOCUMENTS} open documents per session"
                ),
            );
        }
        ...
        if source.len() > DEFAULT_MAX_SOURCE_BYTES {
            ...
            return resource_diagnostics(
                ...
                version,
                format!(
                    "source is {} bytes; Octoscript accepts at most {DEFAULT_MAX_SOURCE_BYTES} bytes",
                    source.len()
                ),
            );
        }
...
const FUZZ_ADVISORY_CONFIGURATION_SOURCE: &str = concat!(
    ...
    "let normalized = text.trim(\"  octoscript  \")\n",
    "text.replace_all(normalized, \"octoscript\", \"Octoscript\")\n",
    "text.slice(normalized, 0, text.len(normalized))\n",
    "text.index_of(normalized, \"octoscript\")\n",
    "text.last_index_of(normalized, \"octoscript\")\n",
...
/// does not start stdio, read a URI, resolve modules, evaluate Octoscript, or
...
pub fn fuzz_exercise_document(source: &str) {
    ...
    let Ok(uri) = Uri::from_str("file:///octoscript-fuzz/document.octoscript") else {
        return;
    };

    let mut server = OctoscriptLanguageServer::with_completion_catalogs_and_workflow_data(
        ToolCompletionCatalog::default(),
...
pub fn fuzz_exercise_advisory_configuration(settings: &serde_json::Value) {
    ...
    let Ok(uri) = Uri::from_str("file:///octoscript-fuzz/configuration.octoscript") else {
        return;
    };
...
#[cfg(fuzzing)]
fn fuzz_exercise_advisory_configuration_requests(server: &OctoscriptLanguageServer, uri: &Uri) {
...
#[cfg(fuzzing)]
fn fuzz_exercise_semantic_requests(server: &OctoscriptLanguageServer, uri: &Uri, source: &str) {
...
fn fuzz_exercise_fixed_core_import_path_requests(
    server: &mut OctoscriptLanguageServer,
    uri: &Uri,
    first_version: i32,
) {
...
fn completion_catalogs_from_initialize_options(
    ...
    let Some(octoscript) = options.get("octoscript") else {
        return (
            ToolCompletionCatalog::default(),
            ModuleCompletionCatalog::default(),
            WorkflowDataCompletionCatalog::default(),
    ...
    let Some(octoscript) = octoscript.as_object() else {
        return (
    ...
    let tool_catalog = match octoscript.get("toolCatalog") {
    ...
    let module_catalog = match octoscript.get("moduleCatalog") {
    ...
    let mut workflow_data_catalog = match octoscript.get("workflowDataCatalog") {
    ...
    let workflow_data_step_context = match octoscript.get("workflowDataStepContext") {
...
/// Extracts the optional `settings.octoscript` object from a standard LSP
...
fn octoscript_settings_from_configuration(
    settings: &serde_json::Value,
) -> Result<Option<&serde_json::Map<String, serde_json::Value>>, ()> {
    let Some(settings) = settings.as_object() else {
        return Err(());
    };
    let Some(octoscript) = settings.get("octoscript") else {
        return Ok(None);
    };
    octoscript.as_object().map(Some).ok_or(())
}
...
fn advisory_catalog_configuration_update_from_settings<Catalog>(
    ...
    let octoscript = match octoscript_settings_from_configuration(settings) {
        Ok(Some(octoscript)) => octoscript,
        Ok(None) => return AdvisoryCatalogConfigurationUpdate::Keep,
        Err(()) => return AdvisoryCatalogConfigurationUpdate::Clear,
    };
    let Some(value) = octoscript.get(key) else {
...
fn workflow_data_configuration_update_from_settings(
    ...
    let (Some(catalog), Some(context)) = (
        octoscript.get("workflowDataCatalog"),
        octoscript.get("workflowDataStepContext"),
    ) else {
        return if octoscript.contains_key("workflowDataCatalog")
            || octoscript.contains_key("workflowDataStepContext")
        {
...
fn handle_request(
    ...
    let response = if request.method == Formatting::METHOD {
        ...
        Response::new_err(
            ...
            format!("unsupported Octoscript LSP request `{}`", request.method),
        )
    };
...
fn syntax_diagnostics(uri: &Uri, source: &str) -> Vec<Diagnostic> {
    match check_syntax_named(uri.as_str(), source, ExecutionLimits::default()) {
        Ok(report) => {
            ...
            if report.diagnostics_truncated {
                diagnostics.push(Diagnostic::new(
                    ...
                    format!("Octoscript stopped after {MAX_SYNTAX_DIAGNOSTICS} syntax diagnostics"),
                    None,
                    None,
                ));
            }
        ...
        Err(error) => vec![Diagnostic::new(
            ...
            None,
            Some(DIAGNOSTIC_SOURCE.to_owned()),
            format!("Octoscript syntax check failed: {error}"),
            None,
            None,
        )],
    }
...
fn direct_module_output_binding(
    ...
    let assignment =
        skip_octoscript_trivia_before(source, symbol.definition.end_byte, initializer_limit_byte)?;
    ...
    let initializer_start =
        skip_octoscript_trivia_before(source, after_assignment, initializer_limit_byte)?;
...
fn skip_octoscript_trivia_before(source: &str, mut index: usize, end_byte: usize) -> Option<usize> {
...
///
/// `workflow` is intentionally not a Octoscript binding. A host must supply a
...
fn direct_module_output_reference_is_stable_read(source: &str, reference: SourceSpan) -> bool {
    let Some(next) = skip_octoscript_trivia(source, reference.end_byte) else {
        return false;
    };
...
fn named_function_parameter_names(
    ...
    let bytes = source.as_bytes();
    let mut index =
        skip_octoscript_trivia_before(source, function.definition.end_byte, maximum_end_byte)?;
    if bytes.get(index) != Some(&b'(') {
        return None;
    }
    index += 1;
    index = skip_octoscript_trivia_before(source, index, maximum_end_byte)?;
    ...
    if bytes.get(index) == Some(&b')') {
        ...
        loop {
            ...
            index = skip_octoscript_trivia_before(source, parameter_end, maximum_end_byte)?;
...
fn imported_module_alias_reference_is_direct_member_call(
    ...
    loop {
        let Some(separator) = skip_octoscript_trivia(source, index) else {
            return false;
        };
        if bytes.get(separator) != Some(&b'.') {
            return false;
        }
        let Some(member_start) = skip_octoscript_trivia(source, separator + 1) else {
            return false;
        };
        if !bytes
            .get(member_start)
        ...
        let Some(next) = skip_octoscript_trivia(source, index) else {
            return false;
        };
...
fn reference_may_mutate_binding(source: &str, reference: SourceSpan) -> bool {
    let bytes = source.as_bytes();
    let Some(mut index) = skip_octoscript_trivia(source, reference.end_byte) else {
        return true;
    };
...
fn skip_octoscript_trivia(source: &str, mut index: usize) -> Option<usize> {
    let bytes = source.as_bytes();
    loop {
        while bytes
            .get(index)
...
mod tests {
    ...
    use octoscript_core::MAX_STATIC_RECORD_ALIASES;

    use super::*;

    fn test_uri() -> Uri {
        Uri::from_str("file:///workspace/example.octoscript").expect("valid file URI")
    }
    ...
    fn reports_syntax_diagnostics_without_executing_source() {
        let mut server = OctoscriptLanguageServer::default();
    ...
    fn completes_exact_visible_mod_tool_members_with_replacement_edits() {
        ...
        let mut partial_server = OctoscriptLanguageServer::default();
    ...
    fn completes_fixed_standard_math_members_without_host_metadata() {
        ...
        let mut server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
        ...
        assert_eq!(
            ...
            Some(Documentation::MarkupContent(MarkupContent {
                kind: MarkupKind::PlainText,
                value: "math.sqrt(value) -> number\n\nReturns a scalar square root.\n\nEffect-free Octoscript core helper; it does not access the host or grant authority.".to_owned(),
            }))
        );
    ...
    fn completes_fixed_standard_json_members_without_host_metadata() {
        ...
        assert_eq!(
            ...
            Some(Documentation::MarkupContent(MarkupContent {
                kind: MarkupKind::PlainText,
                value: "json.parse(document) -> JSON value\n\nParses strict JSON from a string or UTF-8 byte array through Octoscript's configured byte and nesting limits.\n\nBounded Octoscript core data helper; it does not access the host or grant authority.".to_owned(),
            }))
        );
    ...
    fn presents_fixed_standard_json_hover_and_signature_help_without_authority() {
        ...
        let mut hostile_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            hostile_catalog,
        );
    ...
    fn completes_fixed_standard_object_members_without_host_metadata() {
        ...
        assert_eq!(
            ...
            Some(Documentation::MarkupContent(MarkupContent {
                kind: MarkupKind::PlainText,
                value: "object.merge(left, right) -> object\n\nReturns a shallow merge that preserves first field positions and applies right-side values.\n\nBounded Octoscript core object helper; it does not access the host or grant authority.".to_owned(),
            }))
        );
    ...
    fn completes_fixed_standard_text_members_without_host_metadata() {
        ...
        assert_eq!(
            ...
            Some(Documentation::MarkupContent(MarkupContent {
                kind: MarkupKind::PlainText,
                value: "text.trim(value) -> string\n\nRemoves leading and trailing Unicode whitespace.\n\nBounded Octoscript core text helper; it does not access the host or grant authority.".to_owned(),
            }))
        );
    ...
    fn presents_fixed_standard_text_hover_and_signature_help_without_authority() {
        let source = concat!(
            "use mod.std.text\n",
            "let value = text.trim(\"  Octoscript  \")\n",
        ...
        let trim_cursor = source.find("Octoscript").expect("trim input exists") + 1;
    ...
    fn presents_fixed_standard_assert_hover_and_signature_help_without_authority() {
        ...
        let mut direct_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            module_catalog(serde_json::json!([
                {
                    "path": "mod.std.assert",
        ...
        let mut namespace_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            module_catalog(serde_json::json!([
                {
                    "path": "mod.std.assert",
        ...
        let mut alias_server = OctoscriptLanguageServer::default();
        ...
        let mut shadowed_server = OctoscriptLanguageServer::default();
        ...
        let mut unrelated_server = OctoscriptLanguageServer::default();
    ...
    fn presents_visible_named_function_signature_help_without_runtime_access() {
        ...
        let mut zero_server = OctoscriptLanguageServer::default();
        ...
        let mut incomplete_server = OctoscriptLanguageServer::default();
        ...
        for source in [
            ...
            let mut rejected_server = OctoscriptLanguageServer::default();
        ...
        let mut crowded_server = OctoscriptLanguageServer::default();
    ...
    fn presents_visible_named_function_hover_signature_without_runtime_access() {
        ...
        let mut forward_server = OctoscriptLanguageServer::default();
    ...
    fn completes_and_navigates_bounded_static_record_nested_fields() {
        ...
        let mut nested_server = OctoscriptLanguageServer::default();
        ...
        let mut child_alias_server = OctoscriptLanguageServer::default();
        ...
        let mut nested_child_alias_server = OctoscriptLanguageServer::default();
        ...
        let mut nested_child_alias_chain_server = OctoscriptLanguageServer::default();
        ...
        let mut compact_nested_alias_server = OctoscriptLanguageServer::default();
        ...
        for unsupported_source in [
            ...
            let mut unsupported_server = OctoscriptLanguageServer::default();
        ...
        let mut duplicate_child_server = OctoscriptLanguageServer::default();
    ...
    fn follows_bounded_direct_static_record_alias_chains_with_lexical_shadowing() {
        let transitive_source = "let profile = {name: \"Ada\"}\n\
                                 ...
                                 let second = first\n\
                                 second.name";
        let mut transitive_server = OctoscriptLanguageServer::default();
        ...
        let mut shadowed_child_server = OctoscriptLanguageServer::default();
        ...
        let mut indirect_server = OctoscriptLanguageServer::default();
        ...
        let mut depth_server = OctoscriptLanguageServer::default();
        ...
        let mut too_deep_server = OctoscriptLanguageServer::default();
    ...
    fn suppresses_static_record_fields_after_a_mutation_but_not_a_read() {
        ...
        let mut read_server = OctoscriptLanguageServer::default();
    ...
    fn completes_and_hovers_bounded_advisory_workflow_data() {
        ...
        for (source, expected) in [
            ...
            let mut server = OctoscriptLanguageServer::with_workflow_data_catalog(catalog.clone());
            server.open_document(document(1, source));
            let completion = server
                .completion(&test_uri(), position_at_byte(source, source.len()))
        ...
        let mut hover_server = OctoscriptLanguageServer::with_workflow_data_catalog(catalog);
        ...
        let mut deeper_server = OctoscriptLanguageServer::with_workflow_data_catalog(
    ...
    fn accepts_the_workflow_crates_runtime_confirmed_dataflow_projection() {
        ...
        let octoscript = serde_json::to_value(&projection).expect("projection serializes");

        let params = InitializeParams {
            initialization_options: Some(serde_json::json!({"octoscript": octoscript})),
            ..InitializeParams::default()
        };
        ...
        let mut server = OctoscriptLanguageServer::with_workflow_data_catalog_and_step_context(
            catalog,
            step_context,
        );
        server.open_document(document(1, source));
    ...
    fn refreshes_workflow_data_configuration_atomically_and_fails_closed() {
        ...
        let mut malformed_params_server =
            OctoscriptLanguageServer::with_completion_catalogs_and_workflow_data(
    ...
    fn workflow_data_projection_fails_closed_and_respects_shadowing() {
        ...
        let source = "workflow.input.";
        let mut invalid_server = OctoscriptLanguageServer::with_workflow_data_catalog(invalid);
        invalid_server.open_document(document(1, source));
        ...
        let mut absent_server = OctoscriptLanguageServer::default();
    ...
    fn completes_bounded_catalog_names_for_direct_visible_tool_calls() {
        ...
        let mut server = OctoscriptLanguageServer::with_tool_catalog(catalog);
        server.open_document(document(1, source));
    ...
    fn refuses_catalog_names_outside_a_direct_visible_tool_literal() {
        ...
        for source in [
            ...
            let mut server = OctoscriptLanguageServer::with_tool_catalog(catalog.clone());
            server.open_document(document(1, source));
            let completion = server
                .completion(&test_uri(), position_at_byte(source, cursor))
                .expect("completion request succeeds");
    ...
    fn catalog_projection_is_bounded_and_fails_closed_when_malformed() {
        ...
        let source = "use mod.tool\nlet result = tool.call(\"text.\")";
        let mut server = OctoscriptLanguageServer::with_tool_catalog(invalid);
        server.open_document(document(1, source));
        let completion = server
            .completion(
                &test_uri(),
    ...
    fn refreshes_tool_and_module_catalogs_independently_and_fails_closed() {
        ...
        handle_notification(
            ...
            Notification::new(
                DidChangeConfiguration::METHOD.to_owned(),
                DidChangeConfigurationParams {
                    settings: serde_json::json!({"octoscript": []}),
                },
            ),
        )
        .expect("malformed octoscript configuration is handled");
        assert!(server.tool_catalog.unavailable);
        assert!(server.module_catalog.unavailable);
        assert!(server.workflow_data_catalog.unavailable);
        assert!(server.workflow_data_step_context.unavailable);
    ...
    fn completes_bounded_advisory_module_paths_and_direct_imported_members() {
        ...
        let mut import_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        assert_eq!(
            ...
            Some("Octoscript core namespace; fixed and effect-free")
        );
        assert_eq!(
            std_import.documentation,
            Some(Documentation::MarkupContent(MarkupContent {
                kind: MarkupKind::PlainText,
                value: "Fixed Octoscript core namespace. Its presence does not resolve a host module or grant authority.".to_owned(),
            }))
        );

        let nested_import_source = "use mod.std.";
        let mut nested_import_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        assert_eq!(
            ...
            Some("Octoscript core function; fixed assertion helper")
        );

        let member_source = "use mod.std\nstd.";
        let mut member_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut chained_member_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            chained_catalog,
        );
        ...
        let tool_source = "use mod.tool\ntool.";
        let mut tool_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            module_catalog(serde_json::json!([
    ...
    fn completes_fixed_core_import_paths_without_host_metadata_and_masks_fixed_leaves() {
        ...
        let mut root_server = OctoscriptLanguageServer::default();
        ...
        let mut standard_server = OctoscriptLanguageServer::default();
        ...
        assert!(standard_completion.items.iter().all(|item| {
            item.kind == Some(CompletionItemKind::MODULE)
                && item
                    .detail
                    .as_deref()
                    .is_some_and(|detail| detail.starts_with("Octoscript core"))
        }));

        let direct_standard_source = "use mod.std\nstd.";
        let mut direct_standard_server = OctoscriptLanguageServer::default();
        ...
        let mut direct_standard_hover_server = OctoscriptLanguageServer::default();
        ...
        assert_eq!(
            ...
            HoverContents::Markup(MarkupContent {
                kind: MarkupKind::PlainText,
                value: "Fixed Octoscript core scalar helpers. They do not resolve a host module or grant authority.".to_owned(),
            })
        );
        ...
        let mut hostile_member_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            extension_catalog,
        );
        ...
        let mut unavailable_root = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            unavailable.clone(),
        );
        ...
        let mut unavailable_standard = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            unavailable.clone(),
        );
        ...
        let mut unavailable_leaf = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            unavailable,
        );
    ...
    fn hovers_advisory_direct_module_leaves_without_authority() {
        ...
        let mut shadowed_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            module_catalog(serde_json::json!([
                {
                    "path": "mod.arithmetic.remote_add",
        ...
        let mut invalid_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            module_catalog(serde_json::json!([
                {
                    "path": "mod.arithmetic.remote_add",
    ...
    fn presents_bounded_capability_signature_help_without_authority() {
        ...
        let mut mode_only_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            module_catalog(serde_json::json!([
        ...
        let mut incomplete_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            module_catalog(serde_json::json!([
                {
                    "path": "mod.arithmetic.remote_add",
        ...
        let mut comment_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            module_catalog(serde_json::json!([
                {
                    "path": "mod.arithmetic.remote_add",
    ...
    fn catalog_metadata_follows_stable_exact_root_module_aliases() {
        ...
        let mut shadowed_import_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut nested_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        nested_server.open_document(document(1, nested_source));
        ...
        for index in 0..octoscript_core::MAX_IMPORTED_MODULE_ALIAS_DEPTH {
            let alias = format!("alias_{index}");
            bounded_source.push_str(&format!("let {alias} = {previous}\n"));
            previous = alias;
        }
        bounded_source.push_str(&format!("{previous}."));
        let mut bounded_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog,
        );
    ...
    fn catalog_module_aliases_fail_closed_and_never_alias_mod_tool() {
        ...
        let mut over_depth_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        for index in 0..=octoscript_core::MAX_MODULE_IMPORTS {
            truncated_import_source.push_str(&format!("use mod.module_{index}\n"));
        }
        truncated_import_source.push_str("let math = arithmetic\nmath.");
        let mut truncated_import_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut direct_import_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut truncated_alias_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut truncated_input_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut truncated_output_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog,
        );
        ...
        let mut tool_alias_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            ModuleCompletionCatalog::default(),
        );
        ...
        let mut tool_alias_signature_server = OctoscriptLanguageServer::default();
    ...
    fn completes_advisory_direct_module_input_record_fields_without_authority() {
        ...
        let mut empty_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut duplicate_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut malformed_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
    ...
    fn completes_one_nested_advisory_direct_module_input_field_level() {
        ...
        let mut hover_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        hover_server.open_document(document(1, hover_source));
        ...
        let mut sibling_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut alias_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        alias_server.open_document(document(1, alias_source));
        ...
        let mut scalar_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog,
        );
    ...
    fn completes_and_hovers_advisory_direct_module_output_fields_without_authority() {
        ...
        let mut empty_member_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut unavailable_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            unavailable_module_completion_catalog(),
        );
        ...
        let mut deferred_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut deferred_alias_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut alias_hover_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut root_after_alias_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut later_alias_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut bounded_later_alias_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut bounded_alias_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut truncated_root_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog.clone(),
        );
        ...
        let mut no_output_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            no_output_catalog,
        );
    ...
    fn completes_and_hovers_one_nested_advisory_direct_module_output_field_level() {
        ...
        let mut deeper_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            catalog,
        );
        deeper_server.open_document(document(1, deeper_source));
    ...
    fn refuses_advisory_module_completion_outside_direct_visible_contexts() {
        ...
        let mut chained_shadowed_server = OctoscriptLanguageServer::with_completion_catalogs(
            ToolCompletionCatalog::default(),
            module_catalog(serde_json::json!([
    ...
    fn completion_reports_independent_truncation_to_the_client() {
        ...
        for _ in 0..=octoscript_core::MAX_LEXICAL_COMPLETION_SITES {
    ...
    fn rejects_renames_that_are_invalid_ambiguous_or_change_binding_resolution() {
        ...
        for invalid in [
            ...
            assert!(error.contains("canonical Octoscript identifier"), "{error}");
        }
