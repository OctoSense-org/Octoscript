...
//! Host-neutral execution primitives for Octoscript.
//!
//! This crate masks the vendored VM down to the standalone Octoscript source
...
///
/// It lives in `octoscript-ui-l0` now: it depends on `serde_json` alone, and an
...
/// by living here.
pub use octoscript_ui_l0 as ui_l0;
...
/// Octoscript evaluation.
pub const CANONICAL_PROFILE_ID: &str = "octoscript-v0.2";
...
/// Default maximum byte length for one newly constructed Octoscript string.
///
...
/// Default retained-capacity budget for Octoscript-owned VM heap data.
///
...
/// Default maximum live operand values in one Octoscript VM thread.
///
...
/// Default maximum active Octoscript VM call frames, including the root frame.
///
...
///
/// Canonical Octoscript applies this to grammar nesting. The trusted Makepad
...
/// to and including this value is exactly representable by a Octoscript `f64`
/// scalar.
...
pub struct ExecutionLimits {
    ...
    /// Maximum retained capacity tracked for Octoscript-owned VM heap data.
    ///
...
/// Bounds applied to direct Octoscript [`Runtime`] JSON methods.
///
/// The inherited Makepad JSON methods are appropriate for trusted UI hosts but
/// do not apply Octoscript's byte, depth, or cycle rules. The Octoscript runtime
        ...
        match self {
            ...
            Self::SyntaxRejected(report) => {
                let detail = report.diagnostics.first().map_or_else(
                    || "source is not valid canonical Octoscript".to_owned(),
                    |diagnostic| {
                        format!(
                            "line {}, column {}: {}",
                ...
                write!(formatter, "canonical Octoscript syntax rejected: {detail}")
            }
            Self::JsonData(error) => write!(formatter, "JSON data boundary rejected: {error}"),
            Self::EvaluationInProgress => {
                formatter.write_str("a suspended Octoscript evaluation must be resumed first")
            }
...
/// Rejection at a bounded JSON boundary between a Rust host and Octoscript.
///
    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NonFiniteNumber => {
                formatter.write_str("a Octoscript non-finite number cannot cross a JSON boundary")
            }
            ...
            Self::UnsupportedScriptValue => {
                formatter.write_str("a Octoscript value cannot be represented as JSON")
            }
            Self::NonStringObjectKey => {
                formatter.write_str("a Octoscript object has a non-string JSON key")
            }
            Self::UnknownObjectKey => {
                formatter.write_str("a Octoscript object key has no stable string spelling")
            }
            Self::DuplicateObjectKey => {
                formatter.write_str("a Octoscript object has duplicate JSON keys")
            }
            Self::CyclicScriptValue => {
                formatter.write_str("a cyclic Octoscript value cannot cross a JSON boundary")
            }
        }
    }
}
...
/// Result of validating canonical Octoscript source without executing opcodes or
/// tools.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// A bounded, effect-free outline item for valid canonical Octoscript source.
///
...
/// The grammar role that introduced one lexical Octoscript binding.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// One half-open UTF-8 byte span in canonical Octoscript source.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// A single-threaded Octoscript VM with owned host and standard-library state.
///
...
impl<H: Any, S: Any> Runtime<H, S> {
    ...
    /// Injects a host-owned JSON value under one identifier for later Octoscript
    /// evaluations. The value is copied into the VM; Octoscript code cannot retain
    ...
    /// Converts a completed Octoscript value to bounded JSON without constructing
    ...
    /// Validates canonical Octoscript source without evaluating it or entering any
    /// host binding.
    ///
    /// This is suitable for LLM preflight and editor validation. It checks the
    /// portable Octoscript v0.2 grammar, then confirms VM compatibility. Imports,
    ...
    pub fn check_syntax(&self, source: &str) -> Result<SyntaxReport, RuntimeError> {
        check_syntax_named("inline.octoscript", source, self.limits)
    }
    ...
    ///
    /// This accepts syntax outside the portable Octoscript v0.2 profile and is
    ...
    pub fn check_vm_compatibility(&self, source: &str) -> Result<SyntaxReport, RuntimeError> {
        check_vm_compatibility_named("inline.octoscript", source, self.limits)
    }

    /// Formats canonical Octoscript source without evaluating it or entering any
    /// host binding.
    ///
    ...
    pub fn format_source(&self, source: &str) -> Result<String, RuntimeError> {
        format_source_named("inline.octoscript", source, self.limits)
    }
    ...
    pub fn tool_call_hint_report(&self, source: &str) -> Result<ToolCallHintReport, RuntimeError> {
        tool_call_hint_report_named("inline.octoscript", source, self.limits)
    }
    ...
    pub fn imported_module_call_hint_report(
        ...
        imported_module_call_hint_report_named("inline.octoscript", source, self.limits)
    }
    ...
    pub fn module_import_report(&self, source: &str) -> Result<ModuleImportReport, RuntimeError> {
        module_import_report_named("inline.octoscript", source, self.limits)
    }
    ...
    pub fn static_record_shape_report(
        ...
        static_record_shape_report_named("inline.octoscript", source, self.limits)
    }
    ...
    pub fn lexical_symbol_report(&self, source: &str) -> Result<LexicalSymbolReport, RuntimeError> {
        lexical_symbol_report_named("inline.octoscript", source, self.limits)
    }
    ...
    pub fn lexical_completion_report(
        ...
        lexical_completion_report_named("inline.octoscript", source, self.limits)
    }

    /// Evaluates source only after it passes the canonical Octoscript v0.2 profile.
    ///
    ...
    /// deliberately needs a Makepad compatibility construct outside Octoscript.
    ...
    ///
    /// This bypasses Octoscript's portable grammar contract and must not receive
    /// LLM-generated or otherwise untrusted source. Prefer [`Self::eval`] for
    /// all normal Octoscript execution. It still applies the bounded, effect-free
    ...
    /// Returns the currently accounted retained capacity of Octoscript-owned VM
...
/// Validates canonical Octoscript source with the default source-size limit without
/// executing it.
pub fn check_syntax(source: &str) -> Result<SyntaxReport, RuntimeError> {
    check_syntax_named("inline.octoscript", source, ExecutionLimits::default())
}
...
/// Octoscript rejects. It does not resolve imports or establish that a host has
...
pub fn check_vm_compatibility(source: &str) -> Result<SyntaxReport, RuntimeError> {
    check_vm_compatibility_named("inline.octoscript", source, ExecutionLimits::default())
}

/// Returns whether `name` is exactly one non-reserved canonical Octoscript
/// identifier.
///
...
pub fn top_level_declarations(source: &str) -> Result<Vec<TopLevelDeclaration>, RuntimeError> {
    top_level_declarations_named("inline.octoscript", source, ExecutionLimits::default())
}
...
pub fn lexical_symbol_report(source: &str) -> Result<LexicalSymbolReport, RuntimeError> {
    lexical_symbol_report_named("inline.octoscript", source, ExecutionLimits::default())
}
...
pub fn lexical_completion_report(source: &str) -> Result<LexicalCompletionReport, RuntimeError> {
    lexical_completion_report_named("inline.octoscript", source, ExecutionLimits::default())
}
...
pub fn module_import_report(source: &str) -> Result<ModuleImportReport, RuntimeError> {
    module_import_report_named("inline.octoscript", source, ExecutionLimits::default())
}
...
pub fn static_record_shape_report(source: &str) -> Result<StaticRecordShapeReport, RuntimeError> {
    static_record_shape_report_named("inline.octoscript", source, ExecutionLimits::default())
}

/// Lists direct source-level `mod.tool` call hints in valid canonical Octoscript
...
pub fn tool_call_hint_report(source: &str) -> Result<ToolCallHintReport, RuntimeError> {
    tool_call_hint_report_named("inline.octoscript", source, ExecutionLimits::default())
}

/// Lists bounded exact member calls on visible `use mod.<path>` bindings and
/// their bounded exact local root-alias chains in valid canonical Octoscript
...
pub fn imported_module_call_hint_report(
    ...
    imported_module_call_hint_report_named("inline.octoscript", source, ExecutionLimits::default())
}

/// Formats canonical Octoscript source with default bounds without evaluating it.
///
...
pub fn format_source(source: &str) -> Result<String, RuntimeError> {
    format_source_named("inline.octoscript", source, ExecutionLimits::default())
}
...
pub mod fuzzing {
    ...
    /// Runs only the canonical Octoscript preflight for differential fuzzing.
...
/// Validates named canonical Octoscript source without executing it.
///
/// This function rejects Makepad compatibility syntax outside the documented
/// Octoscript v0.2 grammar. It lowers validated canonical statement-ending
...
///
/// Unlike [`check_syntax_named`], this does not apply the canonical Octoscript
...
/// named canonical Octoscript source.
///
/// This applies the same source, token, nesting, canonical-profile, and
/// vendored parser-compatibility checks as [`check_syntax_named`]. It reports
...
/// Formats named canonical Octoscript source without evaluating it.
///
...
/// module, creates a capability, or executes Octoscript source.
...
/// Encodes a Octoscript value as bounded JSON for a trusted native binding.
///
...
/// Decodes bounded JSON into a Octoscript value for a trusted native binding.
///
/// The JSON is validated through Octoscript's bounded host-data boundary before
...
/// the standalone Octoscript source surface. The VM still constructs its upstream
/// bootstrap objects, but generated Octoscript can reach only the retained core and
...
fn restrict_vendored_module_surface(
    ...
    // Retained Octoscript core values and the VM-internal `Range` prototype are
...
/// Clears every primitive type method inherited from Makepad before Octoscript
...
/// Installs a small Octoscript-owned scalar math module without restoring the
...
/// Installs a small Octoscript-owned JSON module that reuses the same bounded
...
/// Octoscript deliberately does not retain inherited range operators.
...
fn bounded_json_method_error(vm: &mut vm::ScriptVm, error: RuntimeJsonError) -> vm::ScriptValue {
    match error {
        RuntimeJsonError::TooLarge { .. } | RuntimeJsonError::TooDeep { .. } => {
            script_err_limit!(
                vm.bx.threads.cur_ref().trap,
                "JSON operation exceeded Octoscript runtime limits"
            )
        }
        _ => script_err_unexpected!(
            vm.bx.threads.cur_ref().trap,
            "value cannot cross Octoscript's bounded JSON boundary"
        ),
    }
}
...
mod tests {
    ...
    fn canonical_preflight_shares_the_bounded_vm_validation_result() {
        ...
        let canonical = check_syntax_named("canonical.octoscript", source, limits).unwrap();
        let compatibility =
            check_vm_compatibility_named("canonical.octoscript", source, limits).unwrap();
    ...
    fn vm_compatibility_preflight_rejects_unavailable_rust_values() {
        let report =
            check_vm_compatibility_named("legacy.octoscript", "@(+", ExecutionLimits::default())
                .unwrap();
    ...
    fn vm_compatibility_preflight_rejects_a_partial_field_assignment_without_panicking() {
        let report =
            check_vm_compatibility_named("legacy.octoscript", "@.b-=", ExecutionLimits::default())
                .unwrap();
    ...
    fn vm_compatibility_preflight_accepts_numeric_separators_without_panicking() {
        let report =
            check_vm_compatibility_named("legacy.octoscript", "1_000", ExecutionLimits::default())
                .unwrap();
    ...
    fn vm_compatibility_preflight_rejects_a_malformed_proto_field_assignment_without_panicking() {
        ...
        let report =
            check_vm_compatibility_named("legacy.octoscript", source, ExecutionLimits::default())
                .unwrap();
...
"#;
    ...
    fn completion_visibility_uses_source_end_when_the_token_stream_is_capped() {
        ...
        let report = lexical_completion_report_named("capped.octoscript", source, limits).unwrap();
    ...
    fn canonical_profile_matches_vm_block_comment_terminators() {
        ...
        let profile =
            check_syntax_named("comment.octoscript", source, ExecutionLimits::default()).unwrap();
        let compatibility =
            check_vm_compatibility_named("comment.octoscript", source, ExecutionLimits::default())
                .unwrap();
    ...
    fn canonical_conditions_require_parenthesized_control_expressions() {
        ...
        let report =
            check_syntax_named("condition.octoscript", source, ExecutionLimits::default()).unwrap();
    ...
    fn formatter_obeys_source_and_token_limits() {
        ...
        assert_eq!(
            format_source_named("limited.octoscript", "hello", limits).unwrap_err(),
            RuntimeError::SourceTooLarge {
                actual: 5,
                maximum: 4,
            }
        ...
        assert!(matches!(
            format_source_named("tokens.octoscript", "let value = 1", limits),
            Err(RuntimeError::SyntaxRejected(report)) if !report.valid
        ));
        ...
        let formatted = format_source_named("tokens.octoscript", "let value=1", limits).unwrap();
        assert_eq!(formatted, "let value = 1");
        assert!(
            check_syntax_named("tokens.octoscript", &formatted, limits)
                .unwrap()
                .valid
        );
    }
    ...
    fn formatter_bounds_indentation_expansion() {
        ...
        assert!(
            check_syntax_named("nested.octoscript", &source, limits)
                .unwrap()
                .valid
        );
        assert!(matches!(
            format_source_named("nested.octoscript", &source, limits),
    ...
    fn generated_canonical_corpus_remains_vm_parse_compatible() {
        ...
        for source in sources {
            ...
            profile_accepted += 1;
            let report = check_syntax_named("generated.octoscript", &source, limits).unwrap();
            assert!(
                report.valid,
    ...
    fn bounds_canonical_profile_nesting() {
        ...
        assert!(report.diagnostics.iter().any(|diagnostic| {
            diagnostic
                .message
                .contains("canonical Octoscript nesting exceeds the maximum")
        }));
    }
    ...
    fn canonical_profile_respects_the_configured_nesting_limit() {
        ...
        assert!(rejected.diagnostics.iter().any(|diagnostic| {
            diagnostic
                .message
                .contains("canonical Octoscript nesting exceeds the maximum of 2")
        }));
    }
    ...
    fn bounds_canonical_profile_token_count_before_vm_parsing() {
        ...
        let report = check_syntax_named("tokens.octoscript", ";;;;", limits).unwrap();
        ...
        assert!(report.diagnostics[0]
            .message
            .contains("canonical Octoscript token count exceeds the maximum of 3"));
    }
    ...
    fn bounds_newline_tokens_emitted_from_block_comments() {
        ...
        let report = check_syntax_named("comment.octoscript", "/*\n\n\n\n*/", limits).unwrap();
    ...
    fn rejects_makepad_compatibility_syntax_outside_the_profile() {
        for (source, expected) in [
            ...
            (
                ...
                "only double-quoted strings are part of the canonical Octoscript profile",
            ),
            (
                "let value = 42u32",
                "numeric literal suffixes are not part of the canonical Octoscript profile",
            ),
            (
                "let values = 1..3",
                "operator `..` is not part of the canonical Octoscript profile",
            ),
    ...
    fn syntax_checks_respect_the_configured_source_limit() {
        ...
        assert_eq!(
            check_syntax_named("limited.octoscript", "hello", limits).unwrap_err(),
            RuntimeError::SourceTooLarge {
                actual: 5,
                maximum: 4,
            }
    ...
    fn rejects_zero_syntax_token_limit() {
        ...
        assert_eq!(
            check_syntax_named("tokens.octoscript", "let value = 1", limits).unwrap_err(),
    ...
    fn rejects_zero_syntax_nesting_limit() {
        ...
        assert_eq!(
            check_syntax_named("nesting.octoscript", "let value = 1", limits).unwrap_err(),
    ...
    fn rejects_zero_string_limit() {
        ...
        assert_eq!(
            check_syntax_named("strings.octoscript", "true", limits).unwrap_err(),
    ...
    fn rejects_zero_heap_limit() {
        ...
        assert_eq!(
            check_syntax_named("heap.octoscript", "true", limits).unwrap_err(),
    ...
    fn rejects_zero_stack_value_limit() {
        ...
        assert_eq!(
            check_syntax_named("stack.octoscript", "true", limits).unwrap_err(),
    ...
    fn rejects_zero_call_frame_limit() {
        ...
        assert_eq!(
            check_syntax_named("frames.octoscript", "true", limits).unwrap_err(),
    ...
    fn rejects_a_time_budget_interval_that_cannot_sample_before_instruction_limit() {
        ...
        assert_eq!(
            check_syntax_named("limits.octoscript", "true", limits).unwrap_err(),
    ...
    fn exposes_frozen_bounded_standard_text() {
        ...
        let report = runtime
            .eval(
                "use mod.std.assert\n\
                 ...
                 assert(text.index_of(\"octoscript\", \"z\") == -1)\n\
                 assert(text.index_of(\"octoscript\", \"\") == 0)\n\
                 assert(text.last_index_of(\"a🙂b🙂\", \"🙂\") == 3)\n\
                 assert(text.last_index_of(\"a🙂b🙂\", \"b\") == 2)\n\
                 assert(text.last_index_of(\"octoscript\", \"z\") == -1)\n\
                 assert(text.last_index_of(\"a🙂b\", \"\") == 3)\n\
        ...
        let invalid_index_of = runtime
            .eval("use mod.std.text\ntry text.index_of(\"octoscript\", 1) catch \"invalid\"")
            .unwrap();
        assert!(
        ...
        let invalid_index_of_value = runtime
            .eval("use mod.std.text\ntry text.index_of(1, \"octoscript\") catch \"invalid\"")
            .unwrap();
        assert!(
        ...
        let invalid_last_index_of = runtime
            .eval("use mod.std.text\ntry text.last_index_of(\"octoscript\", 1) catch \"invalid\"")
            .unwrap();
        assert!(
        ...
        let invalid_slice_index = runtime
            .eval("use mod.std.text\ntry text.slice(\"octoscript\", -1, 1) catch \"invalid\"")
            .unwrap();
        assert!(
        ...
        let invalid_slice_range = runtime
            .eval("use mod.std.text\ntry text.slice(\"octoscript\", 3, 2) catch \"range\"")
            .unwrap();
        assert!(
        ...
        let invalid_delimiter = runtime
            .eval("use mod.std.text\ntry text.split(\"octoscript\", \"\") catch \"empty\"")
            .unwrap();
        assert!(
