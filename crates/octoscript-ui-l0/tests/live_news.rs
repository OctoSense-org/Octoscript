use octoscript_ui_l0::{check_ui_l0_named, makepad, SourceBinding};

#[test]
fn live_news_card_is_l0_and_query_changes_invalidate_both_sources() {
    let card = include_str!("fixtures/live-news.card");
    let report = check_ui_l0_named("live-news", card);
    assert!(report.valid, "{:?}", report.diagnostics);
    for field in ["query", "language"] {
        assert_eq!(
            octoscript_ui_l0::stale_sources(card, &[field]),
            vec!["stories", "progress"]
        );
    }
    assert!(
        !check_ui_l0_named(
            ...
            &card.replace(
                "count: 3,",
                "endpoint: \"https://untrusted.invalid\", count: 3,"
            )
        )
        .valid
    );
...
fn news_binding_quotes_user_text_and_bounds_rows_and_fields() {
    let mut binding = SourceBinding {
        helper: "sys.news_digest".into(),
        field: "0.summary".into(),
        nested: vec![],
        args: vec![
            ("query".into(), "Apple \" OR sys.stock(1)".into()),
    ...
    assert_eq!(
        call,
        "sys.news_digest(\"Apple \\\" OR sys.stock(1)\", \"zh-CN\", \"items.0.summary\")"
    );
...
fn chinese_search_uses_the_real_field_and_preserves_news_sources() {
    use octoscript_ui_l0::{InstanceStore, NodeValue, RealizeLimits, UiNode, ValueOrigin};
    fn find<'a>(node: &'a UiNode, prop: &str, event: &str) -> Option<&'a UiNode> {
        if node
            .args
            .iter()
            .any(|(p, v)| p == prop && matches!(v, NodeValue::Event(e) if e==event))
        {
            return Some(node);
        }
    ...
    for (prop, event, value) in [
        ("on_tap", "edit_search", ""),
        ("on_commit", "search", "美国伊朗最新冲突"),
    ] {
        let report =
        ...
        let origin = octoscript_ui_l0::event_payload_origin(root, &control.key, event).unwrap();
        if event == "search" {
        ...
        if event == "search" {
            assert!(outcome.stale.iter().any(|s| s == "stories"));
        }
    }
    let report =
        octoscript_ui_l0::realize_with_state(card, &data, &store, RealizeLimits::default());
    assert!(report.complete_root().is_ok());
    assert!(octoscript_ui_l0::source_plan(card)
        .requests
        .iter()
        .any(|s| s.helper == "sys.news_digest"));
}
