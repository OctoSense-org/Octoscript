...
fn live_news_card_is_l0_and_query_changes_invalidate_both_sources() {
    let card = include_str!("../../../../tools/octoscript-research/live-news.card");
    let report = check_ui_l0_named("live-news", card);
    assert!(report.valid, "{:?}", report.diagnostics);
    for field in ["query", "language"] {
        assert_eq!(octoscript_ui_l0::stale_sources(card, &[field]), vec!["stories", "progress"]);
    }
...
fn chinese_search_uses_the_real_field_and_preserves_news_sources() {
    use octoscript_ui_l0::{NodeValue, UiNode, InstanceStore, RealizeLimits, ValueOrigin};
    ...
    for (prop, event, value) in [("on_tap", "edit_search", ""), ("on_commit", "search", "美国伊朗最新冲突")] {
        let report = octoscript_ui_l0::realize_with_state(card, &data, &store, RealizeLimits::default());
        ...
        let outcome = octoscript_ui_l0::dispatch_reporting_with_origin(card, &mut store, &control.key,
    ...
    assert!(octoscript_ui_l0::source_plan(card).requests.iter().any(|s| s.helper == "sys.news_digest"));
}
