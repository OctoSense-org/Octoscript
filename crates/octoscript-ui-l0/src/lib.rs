//! **UI Profile Level 0** — a canonical Octoscript profile for LLM-authored UI.
//!
...
/// Largest source this profile will read. Matches `octoscript-core`'s bound, so a
...
fn lex(source: &str, sink: &mut Diagnostics) -> Option<Vec<Token>> {
    ...
    while i < bytes.len() {
        ...
        // rejected so a Octoscript-syntax card cannot be mistaken for an L0 one.
...
/// Lower a realized tree to the Octoscript DSL a Makepad host renders.
///
/// **This is one backend's lowering and belongs in Octoscript-Makepad**, not here. It
...
pub mod makepad {
    ...
    /// which the host receives as `OctoscriptAction::Notify`.
    ///
...
pub struct SourceRequest {
    ...
    /// function is the HOST's job; octoscript-core never calls anything.
    pub helper: String,
...
/// Lower to the Octoscript DSL data form, reporting what could not be mapped.
///
/// The kinds and attribute names below are the consumer's, established by
/// running `octoscript_render::build` rather than by reading about it. Three facts
/// shaped this and each cost a revert to learn:
///
/// - `octoscript-render`'s tag table is closed. An unknown kind at the root rejects
...
/// checked where the consumer lives: `Octoscript-Makepad` dev-depends on this crate
/// and builds the output through `octoscript_render::build`. That dependency is
/// only possible because this crate was extracted from `octoscript-core` and
...
pub mod kit {
    ...
    /// like a broken kit. Every kit in `Octoscript-Makepad` ends this way.
    pub fn lower(root: &UiNode) -> String {
        let mut out = String::from("// REALIZED from an L0 ledger — do not edit.\n");
    ...
    fn element_unsized(node: &UiNode, depth: usize, out: &mut String) {
        ...
        match node.kind.as_str() {
            ...
            // `kit::lower` -> `_kit.octoscript` -> eval -> `l0_widgets`, so the same
