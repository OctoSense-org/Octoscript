...
//! Capability-scoped worker runtime for Octoscript Rust adapters.
//!
...
/// opaque lease. The scope never comes from a worker frame or Octoscript source. A
...
/// [`octoscript_protocol::WorkerCancellationRequest`]. An adapter may poll it or
...
impl WorkerSession {
        ...
        S: WorkerJournalStore,
    {
        ...
        let binding = octoscript_protocol::OperationCompensationBinding::new(
            request.tool.clone(),
            request.operation_key.clone(),
...
mod tests {
    ...
    #[cfg(feature = "sqlite")]
    use octoscript_storage::{sqlite::AnchoredSqliteStore, VolatileRollbackAnchor};
    use octoscript_storage::{
        ...
        assert!(matches!(
            ...
            Err(AuthenticatedWorkerJournalStoreError::Storage(
                AuthenticatedStoreError::Backend(
                    octoscript_storage::sqlite::AnchoredSqliteStoreError::FencingTokenRejected {
                        current: 2,
                        supplied: 1,
                    }
                )
