...
//! Host-owned workflow state for Octoscript.
//!
...
///
/// This mirrors the `octoscript-lsp` parser limit. The projection intentionally
...
/// Host-injected Octoscript identifier containing the current dataflow context.
pub const WORKFLOW_DATA_GLOBAL: &str = "workflow";

const WORKFLOW_DATA_FINGERPRINT_DOMAIN: &[u8] = b"octoscript-workflow-data-v1";
const WORKFLOW_DATA_CONTRACT_FINGERPRINT_DOMAIN: &[u8] = b"octoscript-workflow-data-contract-v1";
...
impl WorkflowData {
    ...
    /// excluded because it never enters the Octoscript VM.
...
/// boundary.
///
/// A draft holds only ordered step IDs and Octoscript source. It has no runtime
...
/// bounds that ordinary schema tooling can validate. The `x-octoscript` extension
...
pub fn workflow_draft_json_schema() -> JsonValue {
    serde_json::json!({
        ...
        "title": "Octoscript Workflow Draft",
        "description": "A data-only proposal for ordered Octoscript workflow steps.",
        "type": "object",
            ...
            "steps": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "source": {
                            "type": "string",
                            "description": "Canonical Octoscript source reviewed separately from draft decoding.",
                        },
                    },
                },
            },
...
/// an LLM draft, Octoscript source, or checkpoint cannot select, weaken, or
...
/// A bounded, metadata-only `octoscript` configuration for `octoscript-lsp`.
///
...
impl WorkflowDataLspProjection {
    ...
    /// step IDs that cannot be represented as direct Octoscript members.
    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::CurrentStepNotAddressable(step_id) => write!(
                formatter,
                "workflow data projection current step is not a direct Octoscript identifier: {}",
                workflow_error_text(step_id)
            ),
...
impl WorkflowPlan {
    ...
    /// Reviews every step with the default canonical Octoscript limits.
    ///
...
/// process restart, but it cannot resume a Octoscript promise or prove that an
    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::StepRejected { step_id, .. } => {
                write!(
                    formatter,
                    "workflow step rejected by the Octoscript profile: {step_id}"
                )
            }
...
impl WorkflowEngine {
    ...
    /// logical external effect; it must not be derived from Octoscript source or a
    ...
    ///
    /// This is the preferred bridge for `octoscript-protocol` workers. It requires
    ...
    /// ledger mutation before completing a matching external Octoscript promise or
...
fn plan_fingerprint(steps: &[WorkflowStep]) -> String {
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"octoscript-workflow-plan-v1");
...
fn workflow_operation_input_fingerprint(input: &[u8]) -> String {
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"octoscript-workflow-operation-input-v1");
...
fn derived_workflow_operation_key(
    ...
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"octoscript-workflow-operation-key-v1");
...
fn derived_workflow_compensation_key(
    ...
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"octoscript-workflow-compensation-key-v1");
    update_plan_fingerprint_component(&mut hasher, plan_fingerprint.as_bytes());
...
fn review_workflow_steps(
    ...
    for step in steps {
        let file = format!("workflow-step-{}.octoscript", step.id);
...
mod tests {
    ...
    use octoscript_capabilities::{CapabilityLeaseError, CapabilityLeaseGrant, ToolPolicy};
    use octoscript_protocol::{canonical_operation_input_bytes, SessionKey, AUTH_TAG_BYTES};
    ...
    fn plan_review_is_effect_free_and_distinguishes_invalid_source_from_pure_source() {
        ...
        assert_eq!(
            ...
            octoscript_core::ToolCallKind::Call
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            octoscript_core::ToolCallKind::Start
        );
    ...
    fn workflow_review_bounds_aggregate_tool_call_hints() {
        ...
        for index in 0..octoscript_core::MAX_TOOL_CALL_HINTS {
        ...
        for step in &review[..4] {
            assert_eq!(
                step.tool_calls.len(),
                octoscript_core::MAX_TOOL_CALL_HINTS,
                "{}",
                step.step_id
            );
    ...
    fn workflow_draft_schema_matches_the_bounded_wire_contract() {
        ...
        assert_eq!(
            schema["x-octoscript"]["schema_version"],
            serde_json::json!(WORKFLOW_DRAFT_SCHEMA_VERSION)
        );
        assert_eq!(
            schema["x-octoscript"]["maximum_wire_bytes"],
            serde_json::json!(MAX_WORKFLOW_DRAFT_BYTES)
        );
        assert_eq!(
            schema["x-octoscript"]["maximum_aggregate_source_bytes"],
            serde_json::json!(MAX_WORKFLOW_PLAN_SOURCE_BYTES)
        );
        assert_eq!(
            schema["x-octoscript"]["step_ids_must_be_unique"],
            serde_json::json!(true)
        );
        assert_eq!(
            schema["x-octoscript"]["steps_are_ordered"],
            serde_json::json!(true)
        );
        assert_eq!(
    ...
    fn step_capability_leases_retain_the_current_authority_across_external_await() {
        ...
        assert!(engine
            ...
            .audit()
            .iter()
            .all(|event| event.outcome == octoscript_capabilities::AuditOutcome::Allowed));
    }
    ...
    fn workflow_cancels_unawaited_external_work_before_the_next_step() {
        ...
        assert_eq!(
            engine.runtime().audit()[0].outcome,
            octoscript_capabilities::AuditOutcome::Cancelled
        );
    ...
    fn cooperative_cancellation_keeps_workflow_suspended_until_confirmation() {
        ...
        assert_eq!(
            engine.runtime().audit()[0].outcome,
            octoscript_capabilities::AuditOutcome::CancellationRequested
        );
    ...
    fn approved_plan_drives_a_deferred_json_capability_to_completion() {
        ...
        runtime
            .register_json_tool(ToolPolicy::json("math.add"), |request| {
                ...
                Ok(octoscript_capabilities::json!({"total": left + right}))
            })
            .unwrap();
        let mut engine = WorkflowEngine::new(runtime);
        let plan = engine
