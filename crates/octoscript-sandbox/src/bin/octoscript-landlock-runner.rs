...
fn main() {
    eprintln!("octoscript-landlock-runner is supported only on Linux");
    std::process::exit(64);
}
...
fn main() {
    if let Err(error) = linux::run(std::env::args_os().skip(1)) {
        eprintln!("octoscript-landlock-runner: {error}");
...
mod linux {
    ...
    const MAX_ALLOWED_EXECUTABLES: usize =
        octoscript_sandbox::bubblewrap::MAX_LANDLOCK_EXECUTABLE_RUNNER_EXECUTABLES;
    ...
    mod tests {
        ...
        fn parses_a_fixed_command_with_a_deterministic_allowlist() {
            ...
            assert_eq!(
                ...
                BTreeSet::from([
                    PathBuf::from("/opt/octoscript/limit-runner"),
                    PathBuf::from("/opt/octoscript/worker"),
                ])
            );
        ...
        fn rejects_malformed_or_duplicated_staged_seccomp_filters() {
            ...
            let values = vec![
                ...
                OsString::from("/opt/octoscript/worker"),
                OsString::from("--"),
                OsString::from("/opt/octoscript/worker"),
            ];
            assert!(matches!(
        ...
        fn rejects_ambiguous_or_untrusted_execution_targets() {
            assert!(matches!(
                ...
                Err(RunnerError::DuplicateAllowedExecutable { executable })
                    if executable == Path::new("/opt/octoscript/worker")
            ));
            assert!(matches!(
                RunnerConfiguration::parse(arguments(&[
                    "--allow-exec",
                    "/opt/octoscript/worker",
                    "--",
                    "/opt/octoscript/other-worker",
                ])),
                Err(RunnerError::CommandNotAllowed { command })
                    if command == Path::new("/opt/octoscript/other-worker")
            ));
            assert!(matches!(
                RunnerConfiguration::parse(arguments(&[
                    "--allow-exec",
        ...
        fn bounds_the_complete_allowlist() {
            ...
            for index in 0..=MAX_ALLOWED_EXECUTABLES {
                ...
                values.push(OsString::from(format!("/opt/octoscript/program-{index}")));
            }
            values.push(OsString::from("--"));
            values.push(OsString::from("/opt/octoscript/program-0"));
