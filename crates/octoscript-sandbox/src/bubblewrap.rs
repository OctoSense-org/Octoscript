//! Linux Bubblewrap containment policy for a static Octoscript worker.
//!
...
/// instruction limit even after Octoscript's fixed ABI and escape-surface guards.
...
/// Octoscript capability manifest or selected by generated source.
...
pub enum WorkerSeccompProfile {
    ...
    #[default]
    Disabled,
    /// Denies a fixed, Octoscript-owned set of common sandbox-escape operations.
    ///
    ...
    ///
    /// The filter still performs Octoscript's ABI/x32 checks and rejects the fixed
...
/// neither Octoscript values nor serializable policy: a host must construct and
...
/// Host-selected Linux rlimits applied by `octoscript-limit-runner` before worker
/// execution.
///
...
///
/// Use the bundled `octoscript-limit-runner` binary or an equivalent reviewed
...
impl LandlockExecutableRunner {
    ...
    /// `octoscript-landlock-runner` or an equivalent reviewed executable.
    ///
...
///
/// This is trusted host setup, not a Octoscript API. Bind one broker mount to one
...
impl LinuxNetworkBrokerMount {
    ...
    /// `octoscript-capabilities/linux-network-broker` host service to create and
...
/// Octoscript. It bounds every directory selected with the same filesystem project
/// ID, so one project can contain a worker's aggregate durable output across
/// several file-root mount points. Octoscript validates the exact mounted directory
...
/// manifest, tool payload, or Octoscript value. Project ID zero is deliberately
...
/// configuration, never Octoscript values.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl FileRootBinding {
    ...
    /// Octoscript is the same directory Bubblewrap mounts and
...
/// not serializable configuration for generated Octoscript source. All executable
...
impl BubblewrapWorkerPolicy {
    ...
    /// derived from Octoscript source, a resource selector, or a tool payload.
    ...
    /// executable sources must also be readable so Octoscript can retain a
    ...
    /// aggregate persistent-storage quota outside Octoscript. Octoscript cannot verify
    ...
    /// [`LinuxProjectQuota`] and use descriptor-pinned mount sources. Octoscript
    ...
    /// Octoscript transfers its bounded compiler-generated cBPF program through
    ...
    /// filter, and only then executes the fixed inner command. No Octoscript
    ...
    /// Selects a typed, Octoscript-owned seccomp hardening profile.
    ///
    /// A selected profile is compiled by Octoscript and transferred to Bubblewrap
    ...
    ///
    /// The raw syscall numbers remain trusted host configuration. Octoscript adds
    ...
    /// never derived from Octoscript source or an LLM request.
...
impl BubblewrapCommand {
    ...
    /// variables, mount paths, or Octoscript values. If the write fails, this
...
/// Octoscript source cannot inspect, reset, or extend it.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl BubblewrapWorkerWatchdog {
    fn new(
        ...
        let join = match thread::Builder::new()
            .name("octoscript-bwrap-watchdog".to_owned())
            .spawn(move || {
...
pub enum BubblewrapPolicyError {
    ...
    LinuxProjectQuotaRead {
        id: String,
        source: octoscript_linux_project_quota::ProjectQuotaError,
    },
    ...
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::LinuxProjectQuotaGroupChanged { project_id } => write!(
                formatter,
                "Linux project quota {project_id} changed while Octoscript compiled one worker policy"
            ),
            ...
            Self::SeccompAllowlistConflictsWithHardening { syscall } => write!(
                formatter,
                "strict seccomp allowlist syscall {syscall} conflicts with Octoscript's fixed hardening"
            ),
...
mod tests {
    ...
    fn bootstrap(session_id: &str) -> PrivatePipeWorkerBootstrap {
        PrivatePipeWorkerBootstrap::new(
            session_id,
            SessionKey::from_bytes([23; octoscript_protocol::AUTH_TAG_BYTES]).unwrap(),
        )
        .unwrap()
    }
    ...
    fn base_policy(root: &TestDirectory) -> BubblewrapWorkerPolicy {
        ...
        let mut policy = BubblewrapWorkerPolicy::new(bwrap, "/opt/octoscript/worker").unwrap();
        policy.add_runtime_mount(ReadOnlyMount::new(runtime, "/opt/octoscript").unwrap());
        policy
    }
    ...
    fn resource_limit_runner(root: &TestDirectory) -> ResourceLimitRunner {
        ...
        ResourceLimitRunner::new("/opt/octoscript/limit-runner", resource_limits()).unwrap()
    }
    ...
    fn landlock_executable_runner(root: &TestDirectory) -> LandlockExecutableRunner {
        ...
        LandlockExecutableRunner::new("/opt/octoscript/landlock-runner").unwrap()
    }
    ...
    fn compiles_a_networkless_allowlisted_worker_plan() {
        ...
        assert!(has_arguments(
            &arguments,
            &["--ro-bind", runtime.to_str().unwrap(), "/opt/octoscript"]
        ));
        assert!(has_arguments(
            &arguments,
        ...
        assert!(has_arguments(
            &arguments,
            &["--", "/opt/octoscript/worker", "--json-lines"]
        ));
    ...
    fn network_origin_grants_require_one_exact_private_broker_mount() {
        ...
        assert_eq!(
            ...
            Path::new("/run/octoscript-network/broker.sock")
        );
    ...
    fn descriptor_pinned_executables_overlay_landlock_runner_and_allowed_targets() {
        ...
        runner
            .add_allowed_executable("/opt/octoscript/interpreter")
            .unwrap();
        ...
        for program in [
            "/opt/octoscript/worker",
            "/opt/octoscript/landlock-runner",
            "/opt/octoscript/interpreter",
        ] {
            assert!(has_arguments(
                &arguments,
...
broker.settimeout(2)
broker.connect('/run/octoscript-network/broker.sock')
...
"#
    ...
    fn landlock_executable_runner_precedes_fixed_inner_runners() {
        ...
        assert!(!has_arguments(
            &arguments,
            &["--allow-exec", "/opt/octoscript/landlock-runner"]
        ));
    }
    ...
    fn landlock_executable_runner_rejects_invalid_runtime_targets() {
        ...
        let mut policy = base_policy(&root);
        policy.set_landlock_executable_runner(
            LandlockExecutableRunner::new("/opt/octoscript/worker").unwrap(),
        );
        assert!(matches!(
            policy.compile(&manifest([])),
            Err(BubblewrapPolicyError::LandlockExecutableRunnerMatchesWorker { program })
                if program == Path::new("/opt/octoscript/worker")
        ));
        ...
        policy.set_resource_limit_runner(resource_limit_runner(&root));
        policy.set_landlock_executable_runner(
            LandlockExecutableRunner::new("/opt/octoscript/limit-runner").unwrap(),
        );
        assert!(matches!(
            policy.compile(&manifest([])),
            Err(BubblewrapPolicyError::LandlockExecutableRunnerMatchesResourceLimitRunner {
                program
            }) if program == Path::new("/opt/octoscript/limit-runner")
        ));
        ...
        policy.set_landlock_executable_runner(
            LandlockExecutableRunner::new("/opt/octoscript/not-executable").unwrap(),
        );
        assert!(matches!(
            policy.compile(&manifest([])),
            Err(BubblewrapPolicyError::LandlockExecutableRunnerNotExecutable { program })
                if program == Path::new("/opt/octoscript/not-executable")
        ));
        ...
        let mut runner = landlock_executable_runner(&root);
        runner
            .add_allowed_executable("/opt/octoscript/not-executable")
            .unwrap();
        policy.set_landlock_executable_runner(runner);
        assert!(matches!(
            policy.compile(&manifest([])),
    ...
    fn landlock_executable_runner_rejects_duplicate_and_automatic_targets() {
        let mut runner = LandlockExecutableRunner::new("/opt/octoscript/landlock-runner").unwrap();
        assert!(matches!(
            runner.add_allowed_executable("/opt/octoscript/landlock-runner"),
            Err(BubblewrapPolicyError::LandlockAllowedExecutableMatchesRunner { program })
                if program == Path::new("/opt/octoscript/landlock-runner")
        ));
        runner
            .add_allowed_executable("/opt/octoscript/interpreter")
            .unwrap();
        assert!(matches!(
            runner.add_allowed_executable("/opt/octoscript/interpreter"),
            Err(BubblewrapPolicyError::DuplicateLandlockAllowedExecutable { program })
                if program == Path::new("/opt/octoscript/interpreter")
        ));

        let mut bounded = LandlockExecutableRunner::new("/opt/octoscript/landlock-runner").unwrap();
        for index in 0..MAX_LANDLOCK_EXECUTABLE_RUNNER_ADDITIONAL_EXECUTABLES {
            bounded
                .add_allowed_executable(format!("/opt/octoscript/program-{index}"))
                .unwrap();
        }
        assert!(matches!(
            bounded.add_allowed_executable("/opt/octoscript/too-many"),
    ...
    fn resource_limit_runner_must_be_distinct_and_readonly_mounted() {
        ...
        let mut policy = base_policy(&root);
        policy.set_resource_limit_runner(
            ResourceLimitRunner::new("/opt/octoscript/worker", resource_limits()).unwrap(),
        );
        assert!(matches!(
            policy.compile(&manifest([])),
        ...
        File::create(root.path().join("runtime/not-executable")).unwrap();
        policy.set_resource_limit_runner(
            ResourceLimitRunner::new("/opt/octoscript/not-executable", resource_limits()).unwrap(),
        );
        assert!(matches!(
            policy.compile(&manifest([])),
    ...
    fn ephemeral_file_roots_share_all_mount_overlap_checks() {
        ...
        for destination in ["/proc/scratch", "/dev/scratch", "/opt/octoscript/scratch"] {
            let mut policy = base_policy(&root);
            policy
                .add_ephemeral_file_root(
                    "scratch",
    ...
    fn ephemeral_file_root_cannot_supply_the_worker_program() {
        ...
        policy
            .add_ephemeral_file_root(
                "runtime",
                EphemeralFileRoot::new("/opt/octoscript", 1024).unwrap(),
            )
            .unwrap();
    ...
    fn requires_the_worker_program_in_a_readonly_runtime_mount() {
        ...
        let bwrap = root.path().join("bwrap");
        create_executable(&bwrap);
        let policy = BubblewrapWorkerPolicy::new(bwrap, "/opt/octoscript/worker").unwrap();
