...
pub enum HttpEndpointMethod {
    ...
    Get,
    /// A `POST` request containing one JSON object or array supplied by Octoscript.
    Post,
}
...
/// binding.
///
/// Octoscript source cannot construct, inspect, serialize, or receive this value.
...
/// in Octoscript input. Implementors can load from a platform credential store or
/// another reviewed Rust integration. Every resolver error is redacted before
/// it can reach Octoscript source, audit records, or tool descriptors.
...
impl HttpEndpoint {
    ...
    ///
    /// The opaque secret identifier is never published to Octoscript. It is
    ...
    /// Octoscript cannot choose either value. Transport-managed headers, cookies,
...
///
/// Octoscript chooses an opaque identifier and provides an absolute request URL,
...
/// network-origin API.
///
/// This is API-level mediation for Octoscript-initiated requests, not an
...
impl HttpOrigin {
    ...
    /// every script-selectable path at the origin. Octoscript cannot select the
...
/// fixed method. During invocation, Octoscript supplies a bounded absolute URL;
