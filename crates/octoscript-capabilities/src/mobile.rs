//! Static-catalog profile for mobile and embedded Octoscript hosts.
//!
...
//! external-dispatch API. Dynamic Octoscript source can receive bounded host-owned
...
impl MobileRuntimeBuilder {
    /// Creates a builder with Octoscript's standard execution and pending-tool
    ...
    ///
    /// The catalog is consumed at setup, so dynamic Octoscript source can request
    ...
    /// profile.
    ///
    /// The catalog is consumed during setup, so dynamic Octoscript source can
    ...
    /// Octoscript source can choose only the reviewed opaque endpoint identifier;
    ...
    /// profile.
    ///
    /// Dynamic Octoscript source can provide a bounded URL only when its exact
    ...
    /// origin matching; Octoscript cannot select the secret, header, or another
    /// origin.
    #[cfg(feature = "http-endpoint-catalog")]
...
/// Dynamic Octoscript runtime backed by a sealed, app-provided local catalog.
///
...
impl MobileRuntime {
    /// Evaluates canonical Octoscript source against the sealed tool catalog.
    ...
    /// canonical Octoscript evaluation.
    ///
    /// The data limits are derived from the sealed execution profile: JSON is
    /// capped by the smaller of its source-byte limit and Octoscript's 64 KiB data
    /// boundary, and by the smaller of its syntax-nesting limit and Octoscript's
    ...
    /// or operator UI. Octoscript source has no catalog-discovery API.
...
mod tests {
    ...
    use octoscript_core::{RuntimeJsonError, DEFAULT_INSTRUCTION_LIMIT, DEFAULT_MAX_SOURCE_BYTES};
