...
//! host-configured credential-store locators. Octoscript cannot select either a
...
//! [`octoscript_worker::secret_broker::SecretProvider`] callback.
...
use octoscript_worker::secret_broker::{SecretProvider, SecretValue};
...
/// become tool metadata or Octoscript data.
...
/// [`octoscript_worker::secret_broker::CapabilitySecretBroker`] also maps it to a
/// generic adapter-side failure rather than exposing it to Octoscript source.
#[cfg(feature = "platform-keyring-worker-secret-provider")]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
mod tests {
    ...
    fn entries_reject_invalid_opaque_ids_and_credential_locators() {
        ...
        assert_eq!(
            PlatformKeyringSecretEntry::new("release.auth", ".com.example.octoscript", "release")
                .unwrap_err(),
    ...
    fn resolver_rejects_duplicate_ids_locators_and_excess_entries() {
        assert_eq!(
            PlatformKeyringSecretResolver::new(vec![
                entry("release.auth", "com.example.octoscript", "release"),
                entry("release.auth", "com.example.octoscript", "preview"),
            ])
            .unwrap_err(),
        ...
        assert_eq!(
            PlatformKeyringSecretResolver::new(vec![
                entry("release.auth", "com.example.octoscript", "release"),
                entry("preview.auth", "com.example.octoscript", "release"),
            ])
            .unwrap_err(),
    ...
    fn resolver_redacts_all_locator_configuration() {
        let entry = entry("release.auth", "com.example.octoscript", "release-user");
        ...
        for private_value in ["release.auth", "com.example.octoscript", "release-user"] {
